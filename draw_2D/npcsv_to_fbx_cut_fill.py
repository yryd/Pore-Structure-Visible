...
def blender_cube_draw(max_density, len, new_data):
    # Number of cubes.
    count = len
    # Size of grid.
    extents = 10
    ...
    for i in range(0, count):
        ...
        for j in range(0, count):
            ...
            if (num > 0):
                ...
                # Asppend a material.
                current.data.materials.append(bpy.data.materials[f'material_{max_density}'])
            else:
                bpy.ops.mesh.primitive_cube_add(location=(centerx + x, centery + y, centerz + z), size=sz)
                current = bpy.context.object
                current.name = 'Cube ({0}, {1}, {2})'.format(k, j, i)
                current.data.name = 'Mesh ({0}, {1}, {2})'.format(k, j, i)
                current.data.materials.append(bpy.data.materials[f'material_0'])
...
def main(file_path):
    ...
    """创建物体并应用材质"""
    blender_cube_draw(num_max, len, new_data)
    """另存为FBX格式模型"""
    # save_fbx(file_path)
